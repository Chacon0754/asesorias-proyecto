...
async function runQuery(sql, binds = [], opts = {}) {
  ...
  let conn;
  try {
    conn = await oracledb.getConnection(); // in theory this should use the created pool
...
const fileFilter = (req, file, cb) => {
  ...
  if (allowedImageExtensions.includes(fileExtension)) {
    ...
    cb(
      new Error("El archivo debe ser una imagen (JPEG, PNG) o un PDF."),
      false,
    ); // Rechazar el archivo
  }
};
...
app.post("/login", async (req, res) => {
  ...
  const findUserQuery = `
  SELECT 'admin' AS role, admin_id AS id, contrasena AS password, nombre AS name, perfil AS profilePicture, apellido1 AS lastName, rol AS rol FROM administrador
  WHERE correo = :email UNION
  SELECT 'student' AS role, matricula AS id, Contra_alum AS password, nombre AS name, perfil AS profilePicture, ape1 AS lastName, rol AS rol FROM alumnos
  WHERE correo = :email UNION
  SELECT 'teacher' AS role, Id_docente AS id, contra_docente AS password, nombre_doc AS name, perfil AS profilePicture, Apellido AS lastName, rol_doc AS rol FROM docentes
  WHERE correo = :email
`;

  try {
    const sql = findUserQuery;
    const binds = { email };
    const opts = { outFormat: oracledb.OUT_FORMAT_OBJECT };

    const result = await runQuery(sql, binds, opts);
...
app.get("/asesorias/alumno/:id", (req, res) => {
  const query = `
    SELECT a.id_as, h.dia, h.hora_inicio, d.nombre_doc AS nombre_docente, d.Apellido AS apellido_docente, m.N_Mat AS nombre_materia, 
    CASE
  ...
  try {
    ...
    const result = runQuery(query, binds, opts);

    console.log("studentId:", id);
    console.log("Asesorías del alumno obtenidas correctamente:", result.rows);
    res.status(200).json(result.rows);
  } catch (error) {
    console.error("Error al obtener asesorías del alumno:", error);
    res.status(500).json({
      message: "Error al obtener asesorías del alumno",
      error: error.message,
    });
  }
});
  ...
  connection.query(query, [id], (error, results) => {
    if (error) {
      ...
      return res.status(500).json({
        message: "Error al obtener asesorías del docente",
...
app.post("/asesorias", (req, res) => {
  const { id_alumno, id_docente, id_docente_horario, id_materia, modalidad } =
    req.body;
  ...
  connection.beginTransaction((error) => {
    if (error) {
      ...
      return res
        .status(500)
        .json({ message: "Error al crear asesoría", error: error.sqlMessage });
    }
        ...
        connection.query(
          ...
          (error, results) => {
            if (error) {
              return connection.rollback(() => {
                console.error(
                  "Error al actualizar horario del docente: ",
                  error,
                );
                res.status(500).json({
                  message: "Error al crear asesoría",
            ...
            console.log(
              "Horario del docente actualizado correctamente: ",
              results,
            );
            ...
            connection.commit((error) => {
              ...
              res
                .status(201)
                .json({ message: "Asesoría creada correctamente" });
            });
          },
        );
      },
    ...
    connection.query(deleteAsesoriaQuery, [asesoriaId], (error, results) => {
      ...
      connection.query(
        ...
        (error, results) => {
          ...
          console.log(
            "Horraio del docente actualizado correctamente: ",
            results,
          );
          ...
          connection.commit((error) => {
            ...
            res
              .status(200)
              .json({ message: "Asesoría eliminada correctamente" });
          });
        },
      );
    });
...
app.post("/admin", (req, res) => {
  const { admin_id, correo, contraseña, nombre, apellido1, apellido2, rol } =
    req.body;
  const query =
    "INSERT INTO administrador (admin_id, correo, contraseña, nombre, apellido1, apellido2, rol) VALUES (?, ?, ?, ?, ?, ?, ?)";
  const hashedPassword = bcrypt.hashSync(contraseña, 10);
  connection.query(
    query,
    [admin_id, correo, hashedPassword, nombre, apellido1, apellido2, rol],
    (error, results) => {
      if (error) {
        console.error("Error al crear admin ", error);
        return res
          .status(500)
          .json({ message: "Error al crear admin", error: error.sqlMessage });
      }
      res
        .status(201)
        .json({ message: "Admin creado correctamente", data: results });
...
app.get("/admin/:id", (req, res) => {
  ...
  connection.query(query, [id], (error, results) => {
    if (error) {
      ...
      return res.status(500).json({
        message: "Error al obtener datos del  admin ",
        error: error.sqlMessage,
      });
    }
    res.status(200).json(results);
...
app.get("/admin/profile-image/:id", (req, res) => {
  ...
  connection.query(query, [id], (error, results) => {
    if (error) {
      return res
        .status(500)
        .json({ message: "Error en el servidor", error: error.sqlMessage });
    }
    if (results.length === 0) {
...
app.post("/admin/upload-profile/:id", upload.single("perfil"), (req, res) => {
  ...
  connection.query(profilePictureQuery, [perfilPath, id], (error, results) => {
    if (error) {
      ...
      return res.status(500).json({
        message: "Error al subir foto del perfil del administrador",
        error: error.sqlMessage,
      });
    }
    res.status(201).json({
      message: "Foto de perfil del estudiante subida correctamente",
      data: results,
    });
    console.log(
      "Foto de perfil del administrador subida correctamente: ",
      results,
    );
  });
});
...
app.get("/students/carreras", (req, res) => {
  const query =
    "SELECT alumnos. *, carrera.Nombre_Carrera as nombre_carrera, semestre.sem as nombre_semestre FROM alumnos JOIN carrera ON alumnos.programa = carrera.Id_Carreras JOIN semestre ON alumnos.semestre = semestre.id";
...
app.post("/alumnos", (req, res) => {
  ...
  const query =
    "INSERT INTO Alumnos (matricula, nombre, ape1, ape2, programa, semestre, correo, perfil, rol, Contra_alum) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  connection.query(
    query,
    [
      matricula,
    ...
    (error, results) => {
      if (error) {
        ...
        return res
          .status(500)
          .json({ message: "Error al crear alumno", error: error.sqlMessage });
      }
      res
        .status(201)
        .json({ message: "Alumno creado correctamente", data: results });
...
app.post("/student/upload-profile/:id", upload.single("perfil"), (req, res) => {
  ...
  const profilePictureQuery =
    "UPDATE alumnos SET perfil = ? WHERE matricula = ?";
  connection.query(profilePictureQuery, [perfilPath, id], (error, results) => {
    if (error) {
      console.error("Error al subir foto de perfil del estudiante", error);
      return res.status(500).json({
        message: "Error al subir foto del perfil del estudiante",
        error: error.sqlMessage,
      });
    }
    res.status(201).json({
    ...
    console.log(
      "Foto de perfil del estudiante subida correctamente: ",
      results,
    );
  });
});
...
app.put("/alumnos/:matricula", (req, res) => {
  const { nombre, ape1, ape2, programa, semestre, correo, perfil, rol } =
    req.body;
  const { matricula } = req.params;
  const query =
    "UPDATE Alumnos SET nombre = ?, ape1 = ?, ape2 = ?, programa = ?, semestre = ?, correo = ?, perfil = ?, rol =?  WHERE matricula = ?";
  connection.query(
    query,
    [nombre, ape1, ape2, programa, semestre, correo, perfil, rol, matricula],
    (error, results) => {
      if (error) {
        console.error("Error al editar alumno ", error);
        return res
          .status(500)
          .json({ message: "Error al editar alumno", error: error.sqlMessage });
      }
      res
        .status(201)
        .json({ message: "Alumno editado correctamente", data: results });
...
app.delete("/alumnos/:matricula", (req, res) => {
  ...
  connection.query(query, [matricula], (error, results) => {
    if (error) {
      ...
      return res
        .status(500)
        .json({ message: "Error al eliminar alumno", error: error.sqlMessage });
    }
    res
      .status(201)
      .json({ message: "Alumno eliminado correctamente", data: results });
...
app.get("/carreras/ids", (req, res) => {
  ...
  connection.query(query, (error, results) => {
    if (error) {
      ...
      return res
        .status(500)
        .json({ message: "Error al obtener los IDs de las carreras", error });
    }
...
app.post("/carrera", (req, res) => {
  ...
  const query =
    "INSERT INTO carrera (Id_Carreras, Nombre_Carrera) VALUES (?, ?)";
  connection.query(query, [Id_Carreras, Nombre_Carrera], (error, results) => {
    if (error) {
      console.error("Error al insertar carrera ", error);
      return res
        .status(500)
        .json({ message: "Error al agregar carrera", error: error.sqlMessage });
    }
    res
      .status(201)
      .json({ message: "Carrera agregada correctamente", data: results });
  });
});
...
app.put("/carrera/:id", (req, res) => {
  ...
  connection.query(query, [Nombre_Carrera, id], (error, results) => {
    if (error) {
      ...
      return res
        .status(500)
        .json({ message: "Error al editar carrera", error: error.sqlMessage });
    }
    res
      .status(201)
      .json({ message: "Carrera editada correctamente", data: results });
...
app.delete("/carrera/:id", (req, res) => {
  ...
  const updateDocentesQuery =
    "UPDATE docentes SET id_mat_as = 0, id_carrera_mat = 0 WHERE id_carrera_mat = ?";
  ...
  const deleteCourseTeacherQuery =
    "DELETE FROM Docente_Materia WHERE id_materia IN (SELECT Id_Materias FROM materias WHERE N_Carr = ?)";
  const deleteAdvisoryQuery =
    "DELETE FROM asesorias WHERE id_materia IN (SELECT Id_Materias FROM materias WHERE N_Carr = ?)";
  ...
  connection.beginTransaction((error) => {
    ...
    connection.query(updateDocentesQuery, [id], (error, results) => {
      ...
      console.log(
        "Docentes relacionados actualizados correctamente: ",
        results,
      );
      ...
      connection.query(deleteCourseTeacherQuery, [id], (error, results) => {
        if (error) {
          return connection.rollback(() => {
            console.error(
              "Error al eliminar relaciones de materias del docente: ",
              error,
            );
            res.status(500).json({
              message: "Error al eliminar carrera",
        ...
        console.log(
          "Relaciones de materias eliminadas correctamente: ",
          results,
        );
        ...
        connection.query(deleteAdvisoryQuery, [id], (error, results) => {
          if (error) {
            return connection.rollback(() => {
              console.error(
                "Error al eliminar asesorias relacionadas: ",
                error,
              );
              res.status(500).json({
                message: "error al eliminar asesorias relacionadas ",
                error: error.sqlMessage,
              });
            });
          }
          ...
          console.log(
            "Asesorias relacionadas eliminadas correctamenre: ",
            results,
          );
          ...
          connection.query(deleteMateriasQuery, [id], (error, results) => {
            if (error) {
              return connection.rollback(() => {
                console.error(
                  "Error al eliminar materias relacionadas: ",
                  error,
                );
                res.status(500).json({
                  message: "Error al eliminar carrera",
            ...
            console.log(
              "Materias relacionadas eliminadas correctamente: ",
              results,
            );
            ...
            connection.query(deleteCarrerasQuery, [id], (error, results) => {
              ...
              connection.commit((error) => {
                ...
                res.status(200).json({
                  message:
                    "Carrera eliminada correctamente, materias relacionadas eliminadas correctamente y docentes relacionados actualizados correctamente",
                });
...
app.get("/materias/carreras", (req, res) => {
  const query =
    "SELECT materias.*, carrera.Nombre_Carrera as nombre_carrera, semestre.sem as nombre_semestre FROM materias JOIN carrera ON materias.N_Carr = carrera.Id_Carreras JOIN semestre ON materias.N_Sem = semestre.id";
  connection.query(query, (error, results) => {
    if (error) return res.status(500).send(error);
    res.json(results);
  });
...
app.get("/materias/:N_Carr", (req, res) => {
  ...
  connection.query(
    "SELECT * FROM Materias WHERE N_Carr = ?",
    [N_Carr],
    (error, results) => {
...
app.post("/materias", (req, res) => {
  ...
  const query =
    "INSERT INTO Materias (Id_Materias, N_Carr, N_Sem, N_Mat) VALUES (?, ?, ?, ?)";
  connection.query(
    query,
    ...
    (error, results) => {
      if (error) {
        console.error("Error al insertar carrera ", error);
        return res.status(500).json({
          message: "Error al agregar materias",
          error: error.sqlMessage,
        });
      }
      res
        .status(201)
        .json({ message: "materia agregada correctamente", data: results });
...
app.put("/materias/:id", (req, res) => {
  ...
  const { id } = req.params;
  const query =
    "UPDATE Materias SET N_Carr = ?, N_Sem = ?, N_Mat = ? WHERE Id_Materias = ?";
  connection.query(query, [N_Carr, N_Sem, N_Mat, id], (error, results) => {
    if (error) {
      console.error("Error al editar carrera ", error);
      return res
        .status(500)
        .json({ message: "Error al editar materia", error: error.sqlMessage });
    }
    res
      .status(201)
      .json({ message: "Materia editada correctamente", data: results });
...
app.delete("/materias/:id", (req, res) => {
  const { id } = req.params;
  const updateDocentesQuery =
    "UPDATE docentes SET id_mat_as = 0 WHERE id_mat_as = ?";
  const deleteCourseTeacherQuery =
    "DELETE FROM Docente_Materia WHERE id_materia = ?";
    ...
    connection.query(updateDocentesQuery, [id], (error, results) => {
      ...
      connection.query(deleteCourseTeacherQuery, [id], (error, results) => {
        if (error) {
          return connection.rollback(() => {
            console.error(
              "Error al eliminar relaciones de la materia: ",
              error,
            );
            res.status(500).json({
              message: "Error al eliminar materia",
        ...
        console.log(
          "Relaciones de la materia eliminadas correctamente: ",
          results,
        );
        ...
        connection.query(deleteAdvisoryQuery, [id], (error, results) => {
          ...
          console.log(
            "Asesorias relacionadas eliminadas correctamente: ",
            results,
          );
          ...
          connection.query(deleteMateriasQuery, [id], (error, results) => {
            ...
            connection.commit((error) => {
              ...
              res.status(200).json({
                message:
                  "Materia eliminada correctamente y docentes actualizados",
              });
...
app.get("/docentes/materia/:id", (req, res) => {
  ...
  connection.query(query, [id], (error, results) => {
    if (error) {
      ...
      return res.status(500).json({
        message: "Error al recuperar docentes",
        error: error.sqlMessage,
      });
    }
    res.json(results);
...
app.post("/docente/upload-profile/:id", upload.single("perfil"), (req, res) => {
  ...
  const profilePictureQuery =
    "UPDATE docentes SET perfil = ? WHERE Id_docente = ?";
  connection.query(profilePictureQuery, [perfilPath, id], (error, results) => {
    if (error) {
      console.error("Error al subir foto de perfil del docente", error);
      return res.status(500).json({
        message: "Error al subir foto del perfil del docente",
        error: error.sqlMessage,
      });
    }
    res.status(201).json({
      message: "Foto de perfil del docente subida correctamente",
      data: results,
    });
...
app.post("/docentes", (req, res) => {
  ...
  const insertTeacherQuery =
    "INSERT INTO docentes (Id_docente, nombre_doc, Apellido, id_mat_as, id_carrera_mat, correo, apei2, perfil, rol_doc, contra_docente) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  const insertCourseTeacherQuery =
    "INSERT INTO Docente_Materia (id_docente, id_materia) VALUES ?";
  const insertHorarioTeacherQuery =
    "INSERT INTO Docente_Horario (id_docente, id_horario, ocupado) VALUES ?";
  ...
  connection.beginTransaction((error) => {
    if (error) {
      ...
      return res
        .status(500)
        .json({ message: "Error al crear docente", error: error.sqlMessage });
    }
    ...
    connection.query(
      ...
      (error, results) => {
        ...
        const courseTeacherValues = courseIds.map((courseId) => [
          Id_docente,
          courseId,
        ]);
        ...
        connection.query(
          insertCourseTeacherQuery,
          [courseTeacherValues],
          (error, results) => {
            if (error) {
              return connection.rollback(() => {
                console.error(
                  "Error al insertar materias del docente: ",
                  error,
                );
                res.status(500).json({
                  message: "Error al crear docente",
            ...
            console.log(
              "Materias del docente insertadas correctamente: ",
              results,
            );

            // Prepara los datos para la tabla intermedia Docente_Horario
            const horarioTeacherValues = scheduleIds.map((horarioId) => [
              Id_docente,
              horarioId,
              false,
            ]);
            ...
            connection.query(
              insertHorarioTeacherQuery,
              [horarioTeacherValues],
              (error, results) => {
                if (error) {
                  return connection.rollback(() => {
                    console.error(
                      "Error al insertar horarios del docente: ",
                      error,
                    );
                    res.status(500).json({
                      message: "Error al crear docente",
                ...
                console.log(
                  "Horarios del docente insertados correctamente: ",
                  results,
                );
                ...
                connection.commit((error) => {
                  ...
                  console.log(
                    "Docente y sus relaciones insertados correctamente.",
                  );
                });
              },
            );
...
app.put("/docentes/:id", (req, res) => {
  ...
  const updateTeacherQuery =
    "UPDATE docentes SET nombre_doc = ?, Apellido = ?, id_carrera_mat = ?, correo = ?, apei2 = ?, perfil = ?, rol_doc = ? WHERE Id_docente = ?";
  const deleteCourseTeacherQuery =
    "DELETE FROM Docente_Materia WHERE id_docente = ?";
  const insertCourseTeacherQuery =
    "INSERT INTO Docente_Materia (id_docente, id_materia) VALUES ?";
  const deleteAdvisoryQuery = "DELETE FROM asesorias where id_docente = ?";
  const deleteScheduleTeacherQuery =
    "DELETE FROM Docente_Horario WHERE id_docente = ?";
  const insertScheduleTeacherQuery =
    "INSERT INTO Docente_Horario (id_docente, id_horario, ocupado) VALUES ?";
  ...
  connection.beginTransaction((error) => {
    if (error) {
      ...
      return res
        .status(500)
        .json({ message: "Error al editar docente", error: error.sqlMessage });
    }
        ...
        connection.query(deleteAdvisoryQuery, [id], (error, results) => {
          if (error) {
            return connection.rollback(() => {
              ...
              res.status(500).json({
                message: "Error al editar al docente ",
                error: error.sqlMessage,
              });
            });
          }
            ...
            connection.query(
              ...
              (error, results) => {
                if (error) {
                  return connection.rollback(() => {
                    console.error(
                      "Error al insertar nuevas relaciones de materias del docente: ",
                      error,
                    );
                    res.status(500).json({
                      message: "Error al editar docente",
                ...
                console.log(
                  "Nuevas relaciones de materias insertadas correctamente: ",
                  results,
                );

                // Elimina las relaciones actuales en la tabla intermedia Docente_Horario
                connection.query(
                  deleteScheduleTeacherQuery,
                  [id],
                  (error, results) => {
                    if (error) {
                      return connection.rollback(() => {
                        console.error(
                          "Error al eliminar relaciones de horarios del docente: ",
                          error,
                        );
                        res.status(500).json({
                          message: "Error al editar docente",
                    ...
                    console.log(
                      "Relaciones de horarios eliminadas correctamente: ",
                      results,
                    );

                    // Prepara los datos para la tabla intermedia Docente_Horario
                    const scheduleTeacherValues = scheduleIds.map(
                      (scheduleId) => [id, scheduleId, false],
                    ); // `false` para `ocupado`
                    console.log(scheduleTeacherValues);

                    // Inserta las nuevas relaciones en la tabla intermedia Docente_Horario
                    connection.query(
                      insertScheduleTeacherQuery,
                      [scheduleTeacherValues],
                      (error, results) => {
                        if (error) {
                          return connection.rollback(() => {
                            console.error(
                              "Error al insertar nuevas relaciones de horarios del docente: ",
                              error,
                            );
                            res.status(500).json({
                              message: "Error al editar docente",
                        ...
                        connection.commit((error) => {
                          ...
                          res
                            .status(201)
                            .json({ message: "Docente editado correctamente" });
                          console.log(
                            "Nuevas relaciones de horarios insertadas correctamente: ",
                            results,
                          );
                        });
                      },
      ...
      connection.query(deleteAdvisoryQuery, [id], (error, results) => {
        ...
        connection.query(deleteScheduleTeacherQuery, [id], (error, results) => {
          if (error) {
            return connection.rollback(() => {
              console.error(
                "Error al eliminar horarios de docente relacionadas: ",
                error,
              );
              res.status(500).json({
                message: "Error al eliminar docente",
          ...
          console.log(
            "Horarios relacionadas eliminadas correctamente: ",
            results,
          );
          ...
          connection.query(deleteTeacherQuery, [id], (error, results) => {
            ...
            connection.commit((error) => {
              ...
              res
                .status(201)
                .json({ message: "Docente eliminado correctamente" });
